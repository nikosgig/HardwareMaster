package hardwaremaster.com.old.widgets;
...
import hardwaremaster.com.old.util.BitmapUtils;
import hardwaremaster.com.old.util.PixelUtils;
...
public class RangeSeekBar<T extends Number> extends AppCompatImageView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int defaultShadowYOffset = PixelUtils.dpToPx(context, 2);
        int defaultShadowXOffset = PixelUtils.dpToPx(context, 0);
        int defaultShadowBlur = PixelUtils.dpToPx(context, 2);

        if (attrs == null) {
            setRangeToDefaultValues();
            internalPad = PixelUtils.dpToPx(context, INITIAL_PADDING_IN_DP);
            barHeight = PixelUtils.dpToPx(context, LINE_HEIGHT_IN_DP);
            ...
            try {
                ...
                if (normalDrawable != null) {
                    thumbImage = BitmapUtils.drawableToBitmap(normalDrawable);
                }
        ...
        textSize = PixelUtils.dpToPx(context, DEFAULT_TEXT_SIZE_IN_DP);
        distanceToTop = PixelUtils.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        textOffset = !showTextAboveThumbs ? 0 : this.textSize + PixelUtils.dpToPx(context,
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight()
                + (!showTextAboveThumbs ? 0 : PixelUtils.dpToPx(getContext(), HEIGHT_IN_DP))
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (showLabels) {
            ...
            String minLabel = getContext().getString(R.string.seek_bar_min_label);
            String maxLabel = getContext().getString(R.string.seek_bar_max_label);
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            if (!singleThumb) {
                ...
                int spacing = PixelUtils.dpToPx(getContext(), TEXT_LATERAL_PADDING_IN_DP);
